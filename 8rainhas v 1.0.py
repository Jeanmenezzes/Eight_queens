...
def criaTabuleiro(tabuleiro_N, tabuleiro_Q):

    #Caso seja a primeira execussão a lista tabuleiro_N estará vazia e PODE HAVER PROBLEMA na execussão do restante do código da função - À VERIFICAR -.
    if tabuleiro_N == []:
        return tabuleiro_Q
    
    #Faz um loop com cada valor referente às posições do tabuleiro das rainhas que serão inseridas no tabuleiro_Q.
    for rainhas_Inseridas in range(len(tabuleiro_N)):
        #Marca o valor da posição da rainha que será inserida e converte para os valores (de lista) referentes à linha e a coluna.
        posicao_Rainha_Tabuleiro = tabuleiro_N[rainhas_Inseridas]
        linha = posicao_Rainha_Tabuleiro // 8
        coluna = (posicao_Rainha % 8) -1
        ...
        for linhaMenos in range(linha -1, -1, -1):
                mult += 1
                #Cria um bloco try pois a variável mult pode atingir valores fora do índice
                try:
                    if coluna - mult >= 0:
...
#
def insereRainha(tabuleiro_N, tabuleiro_Q, contador = 1, casas_Maximas = [64]):
    tabuleiro_Adicionar_N = []
...
def funcaoGeradora(tabuleiro_N, tabuleiro_Q, casas_Maximas):
    ...
    tabuleiro_Q  = criaTabuleiro(proxima_Sequencia_Inicio, tabuleiro_Q)

    
    tabuleiro_N, casas_Maximas = insereRainha(tabuleiro_N, tabuleiro_Q, contador, casas_Maximas)
