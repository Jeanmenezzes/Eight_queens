...
def criaTabuleiro(proxima_Sequencia_Inicio, tabuleiro_Q, contador, casas_Maximas):
    ...
    #ERRO AQUI
    if (contador -1) // 8 == contador // 8 and tabuleiro_Q[contador // 8][(contador%8)-1] == 'Q':
        linha = (contador // 8) + 1
        contador += 8 - coluna
    else:
        linha = contador // 8 
    ...
    for i in range(linha, 8):
        for j in range(8):
            ...
            if tabuleiro_Q[i][j] == 0:
                tabuleiro_Q = insereRainha(contador + 1, tabuleiro_Q)

                tabuleiro_Adicionar_N.append(contador + 1)

                #Após o tabuleiro_Q ter sido completamente implementado pela inserção da rainha de valor igual à variável contador + 1, é criada uma outra
                #variável casa_Maxima que representa o valor máximo que a próxima rainha poderá assumir
                casa_Maxima = False
                contador_Casa_Maxima = contador
                
                for num in range(contador_Casa_Maxima,64):
                    contador_Casa_Maxima += 1
                    if tabuleiro_Q[num//8][num%8] == 0:
                        casa_Maxima = contador_Casa_Maxima
                if not casa_Maxima:
                    pass
                else:
    ...
    return tabuleiro_Adicionar_N, casas_Maximas
...
def funcaoGeradora(tabuleiro_N, tabuleiro_Q, casas_Maximas):
    ...
    tabuleiro_Adicionar_N, casas_Maximas = criaTabuleiro(proxima_Sequencia_Inicio, tabuleiro_Q, contador, casas_Maximas)
...
def main():
    ...
    for j in range(20):
        ...
        aux = funcaoGeradora(tabuleiro_N, tabuleiro_Q, casas_Maximas)
        for i in aux:
            tabuleiro_N, casas_Maximas = i[0], i[1]
            #for i in range(8):
                #print(tabuleiro_Q[i])
...
#PROBLEMA GERAL
#CORRIGIR OS ERROS QUE ESTÃO SENDO GERADOS NAS FUNÇÕES criaTabuleiro --- PROVAVELMENTE É ALGUM PROBLEMA COM A SOMA DA VARIÁVEL CONTADOR.
